...
namespace LibroSueldoDigital.Formularios
{
    public partial class FrmCrear : Telerik.WinControls.UI.RadForm
    {
        ...
        {
            HiloCrearArchivo.RunWorkerAsync();
        ...
        private void CmbSeleccionarArchivo_Click(object sender, EventArgs e)
        {
            if (CrearLibro)
            {
                OfdAbrirArchivo.Filter = "Archivo de Excel (*.xlsm)|*.xlsm| Excel v2 (*.xlsx)|*.xlsx";
            }
            else
            {
                OfdAbrirArchivo.Filter = "Archivo de Excel (*.xlsx)|*.xlsx";
            }
            OfdAbrirArchivo.FileName = "";
        ...
        private void HiloCrearArchivo_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            else if (CrearLibro)
            {
                ...
                for (int C = 3; C < Columnas + 3; C++)// recorre una columna por empleado
                {
                    ...
                    HiloCrearArchivo.ReportProgress(C);
                }
        ...
        private void HiloCrearArchivo_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            PgbProceso.Value1 = e.ProgressPercentage;
        }

        private void HiloCrearArchivo_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            PgbProceso.Value1 = PgbProceso.Maximum;
        }
    }
}
